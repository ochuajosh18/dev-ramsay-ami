...
import { selectModifiedArticles } from "../../../store/articles/selectors";
import { paginate, sort } from "../../../utils/helpers";

const ArticlesTable = ({ rowsPerPage, searchKeyword }: TableOwnProps) => {
  const [page, setPage] = React.useState(0);
  const [sortColumn, setSortColumn] = useState<SortColumn>({
    path: "index",
    order: "asc",
  });
  const data = useSelector(selectModifiedArticles, shallowEqual);

  const columns = React.useMemo<TableColumn[]>(() => articleColumns, []);
  // sort handler
