...
import { addMostPopularPost } from "../../../store/most-popular-posts/actions";
...
const MostPopularForm = () => {
  ...
  const { mediaType, mediaPreview, setMedia } = useMediaPreview();
  const {
    register,
    handleSubmit,
    control,
  ...
  function onSubmit(values: MostPopularInputs) {
    ...
    const newPost = { id, ...values };
    dispatch(addMostPopularPost(newPost));
...
export default MostPopularForm;
