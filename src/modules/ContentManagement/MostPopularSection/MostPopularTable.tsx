...
import mostPopularColumns from "./MostPopularColumns";
import { selectModifiedMostPopularPosts } from "../../../store/most-popular-posts/selectors";
import { paginate, sort } from "../../../utils/helpers";

const MostPopularTable = ({ rowsPerPage, searchKeyword }: TableOwnProps) => {
  const [page, setPage] = React.useState(0);
  const [sortColumn, setSortColumn] = useState<SortColumn>({
    path: "index",
    order: "asc",
  });
  const data = useSelector(selectModifiedMostPopularPosts, shallowEqual);
  const columns = React.useMemo<TableColumn[]>(() => mostPopularColumns, []);
  // sort handler
